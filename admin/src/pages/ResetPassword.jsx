...
const ResetPassword = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      const { data } = await axios.post(`${import.meta.env.VITE_CLIENT_API_URL}/api/admin/forgot-password`, { 
        email 
      });
      if (data.success) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 py-16 max-w-md mx-auto px-4 sm:px-6 ">
      {/* Main Container Card */}
      <div className="w-full max-w-[440px] bg-white dark:bg-stone-900 rounded-[24px] p-10 shadow-[0_10px_30px_rgba(0,0,0,0.04)] border border-[#eeeeee] text-center">
        ...
        <h2 className="font-serif text-white text-2xl uppercase tracking-wide mb-2 font-normal">
          Reset Your Password
        </h2>
        
        <p className="text-[#8f8d8d] text-sm leading-relaxed mb-7">
          Enter your registered email to receive a secure recovery verification.
        </p>

        {/* Form */}
        <form onSubmit={handleSubmit} className="text-left">
          <label className="block text-xs font-semibold uppercase tracking-wider text-[#eee9e9] mb-2">
            Email Address *
          </label>
        ...
        <div className="mt-8 pt-6 border-t border-[#f0f0f0]">
          <Link 
            to="/login" 
            className="inline-flex items-center gap-2 text-xs font-medium text-[#ffffff] hover:text-[#a7aaa8] transition-colors"
          >
            <ArrowLeft size={14} />
